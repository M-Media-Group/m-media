    ...
    <div>
        <transition-group name="fade" mode="out-in">
            ...
            <div class="alert alert-danger" role="alert" v-show="error" key="4">
                ...
                <a href="https://blog.mmediagroup.fr/post/top-level-domains-on-m-media/" target="_BLANK" rel="noopener" class="button">See the top-level domains we support</a>
            </div>
            <form @submit.prevent="getDomain" action="#" method="POST" enctype="multipart/form-data" v-show="!success && !error" key="1" class="mb-0">
                <div class="form-group row">
                    <label for="domain" class="col-md-4 col-form-label text-md-right">Domain</label>

                    <div class="col-md-6">
                        <input name="domain" v-model="domain" v-on:input="availability = null" id="domain" class="form-control" type="text" placeholder="example.com" :disabled="loading" required>
                    </div>
            ...
            <div class="table-responsive" v-if="suggestions && availability && availability !== 'AVAILABLE'" key="suggestions">
                <table class="table">
                        <thead>
                            <tr>
            ...
            <div class="row" key="5">
                <div class="col-md-8 offset-md-4 text-muted">
                    <a href="https://blog.mmediagroup.fr/post/check-if-domain-available-on-m-media/" target="_BLANK" rel="noopener">Need help?</a>
                </div>
            </div>
        </transition-group>
    </div>
...
export default{
    ...
    getDomain: function () {
        ...
        this.loading = true
        //data.append('_method', 'put'); // add this
        axios.get('/api/domains/'+this.domain+'/availability') // change this to post )
        .then(res =>{
        ...
        this.loading = true
        //data.append('_method', 'put'); // add this
        axios.get('/api/domains/'+this.domain+'/suggestions') // change this to post )
        .then(res =>{
