                ...
                <div class="table-responsive" v-if="subscription.items.data" key="items">
                  <table class="table">
                          ...
                          <tr v-for="item in subscription.items.data" :key="item['id']">
                              ...
                              <td>{{item.quantity ? (item.plan.amount * item.quantity / 100) : 0}} {{item.plan.currency.toUpperCase()}} / {{item.plan.interval}}</td>
                          </tr>
                      </tbody>
                  </table>
                </div>
...
export default{
    ...
    getSub: function () {
        ...
        axios.get('/api/subscriptions/'+this.subid) // change this to post )
        .then(res =>{
