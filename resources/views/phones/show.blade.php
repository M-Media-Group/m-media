...
<div class="table-responsive table-hover">
        <table class="table mb-0">
            <tbody>
                <tr>
                    <th>Number</th>
                    <td><a target="_BLANK" rel="noopener noreferrer" href="tel:{{ $phone->e164 }}">{{$phone->e164}}</a></td>
                </tr>
                <tr>
                    <th>Is public</th>
                    <td class="text-{{ $phone->is_public  ? 'secondary' : 'muted' }}">{{ $phone->is_public  ? 'Yes' : 'No' }} </td>
                </tr>

                <tr>
                    <th>Owned by</th>
                    <td class="text-{{ !$phone->user  ? 'secondary' : null }}">{!! $phone->user ? '<a href="/users/'.$phone->user->id.'">'.$phone->user->name."</a>" : 'No owner' !!}</td>
                </tr>
                <tr>
                    <th>Created</th>
    ...
    <p class="mb-5"><a href="/contact">Contact us if you need to change info about this phone.</a></p>
        ...
        @can('delete', $phone)
            <form class="d-inline" method="POST" action="/phones/{{$phone->id}}">
                @csrf
                @method('DELETE')
                <button type="submit" class="button button-primary" onclick="return confirm('Please confirm you want to delete this phone forever.');">
                    {{ __('Delete phone') }}
                </button>
            </form>
        @endif
         <h2 class="mt-5 mb-0" id="phones">Logs</h2>
	    ...
	    @foreach($phone->logs->reverse() as $log)
	            <tr>
	                @if($log->type == 'INBOUND')
						<td>You called us</td>
					@elseif($log->type == 'OUTBOUND_AUTO_SMS')
